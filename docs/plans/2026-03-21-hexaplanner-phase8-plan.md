# HexaRail Phase 8 Implementation Plan: Pipeline Open Data CFF (GTFS & PostGIS)
...
Run: `nix develop -c bash -c "pg_ctl status -D .pgdata || true && psql -p 15432 -d hexarail_dev -c 'CREATE EXTENSION IF NOT EXISTS postgis;'"`
...
- Create: `hexarail/lib/hexarail/repo.ex` (Modify to configure PostGIS types)

**Step 1: Write the failing check**
Run: `nix develop -c bash -c "cd hexarail && mix deps.tree | grep geo_postgis"`
Expected: FAIL

**Step 2: Write minimal implementation**
1. Add `{:geo_postgis, "~> 3.4"}` to `mix.exs`.
2. Update the `HexaRail.Repo` module (if it exists, or ensure it's configured) to know about PostGIS types:
```elixir
# in repo.ex or application config
Postgrex.Types.define(HexaRail.PostgresTypes, [Geo.PostGIS.Extension] ++ Ecto.Adapters.Postgres.extensions(), [])
```
*(Note: We will configure this via the Repo configuration in the Ecto setup).*

**Step 3: Run check to verify it passes**
Run: `nix develop -c bash -c "cd hexarail && mix deps.get && mix compile"`
Expected: PASS

**Step 4: Commit**
```bash
git add hexarail/mix.exs hexarail/mix.lock
...
**Files:**
- Create: `hexarail/priv/repo/migrations/*_create_cff_stops.exs`
- Create: `hexarail/lib/hexarail/cff/stop.ex`

**Step 1: Write the failing test**
Create a test to insert a GTFS Stop (ex: Bern Hauptbahnhof).
```elixir
# hexarail/test/cff/stop_test.exs
defmodule HexaRail.CFF.StopTest do
  use HexaRail.DataCase # Requires standard Ecto DataCase
  ...
  test "insert and retrieve a GTFS stop with geospatial point" do
    ...
    {:ok, stop} = HexaRail.Repo.insert(%HexaRail.CFF.Stop{
...
Run: `nix develop -c bash -c "cd hexarail && mix test test/cff/stop_test.exs"`
Expected: FAIL
...
```
3. Create the Schema `lib/hexarail/cff/stop.ex`:
```elixir
defmodule HexaRail.CFF.Stop do
  use Ecto.Schema
...
Run: `nix develop -c bash -c "cd hexarail && mix ecto.migrate && mix test test/cff/stop_test.exs"`
Expected: PASS
