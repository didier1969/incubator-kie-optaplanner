defmodule HexaRail.KinematicsTest do
  use ExUnit.Case
  alias HexaRail.RailwayNif
  alias HexaRail.Domain.{OsmNode, OsmWay}
  alias HexaRail.Fleet.RollingStockProfile
  ...
  test "Phase 5: Heavy freight train takes longer to route via switches than light passenger train" do
    ...
    RailwayNif.load_fleet(resource, %{1 => freight_profile, 2 => tgv_profile})
    ...
    {freight_path, freight_time} = RailwayNif.route_micro_path_with_kinematics(resource, 1, 3, 1)
    {tgv_path, tgv_time} = RailwayNif.route_micro_path_with_kinematics(resource, 1, 3, 2)
