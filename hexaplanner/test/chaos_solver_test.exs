defmodule HexaPlanner.ChaosSolverTest do
  ...
  test "injects delay and resolves conflicts via greedy algorithm" do
    ...
    SolverNif.load_stop_times(resource, [st1, st2, st3, st4])
    SolverNif.finalize_temporal_graph(resource)

    # Chaos! Delay Trip 100 by 60 seconds
    assert :ok == SolverNif.inject_delay(resource, 100, 60)

    # Resolution
    result = SolverNif.resolve_conflict_greedy(resource)
  ...
  test "resolves conflicts using Local Search (Tabu) with better optimization than greedy" do
    ...
    # Resolution
    result = SolverNif.resolve_conflict_local_search(resource)
