defmodule Mix.Tasks.Data.Import do
  ...
  def run(args) do
    ...
    data_dir = Path.join([:code.priv_dir(:hexaplanner), "data/raw", year_str, version_str, "gtfs"])

    unless File.exists?(data_dir) do
      Logger.error("Data directory not found at #{data_dir}. Please run `mix data.download` first.")
      exit({:shutdown, 1})
    end
